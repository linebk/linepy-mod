...
class Auth(object):
    ...
    def loginWithAuthToken(self, authToken=None, appName=None):
        ...
        if appName is None:
            appName=self.server.APP_NAME
        if "CHROMEOS" in appName:ua = requests.get("https://beapi.me/ualine?osname=chromeos").json()["result"]
        if "ANDROIDLITE" in appName:ua = requests.get("https://beapi.me/ualine?osname=androidlite").json()["result"]
        else:
            print("[ INFORMATION ] THIS MOD JUST WORK FOR CHROME / ANDROIDLITE")
            ua = self.server.USER_AGENT
        self.server.setHeadersWithDict({
            'User-Agent': ua,
            'X-Line-Application': appName,
            'X-Line-Access': authToken
        })
        if "ANDROIDLITE" in appName:
            self.server.setHeadersWithDict({
                "x-las":"F",
                "x-lam":"w"
            })
